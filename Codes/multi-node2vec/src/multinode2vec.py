...
def generate_features(nbrhds, d, out, nbrhd_size=-1, w2v_iter=1, workers=8, sg=1):
    ...
    w2v_model = w2v.Word2Vec(nbrhds, vector_size=d, window=nbrhd_size, min_count=0,
                             workers=workers, epochs=w2v_iter, sg=sg)
    embfile = out + ".emb"
    ...
    w2v_model.wv.index_to_key = [key.replace(' ', '_') for key in w2v_model.wv.index_to_key]
    w2v_model.wv.save_word2vec_format(embfile)
    ftrs = emb_to_pandas(embfile)
    if ftrs is None:
        print("Failed to read embeddings into pandas DataFrame.")
        return None
...
def emb_to_pandas(emb_file):
    try:
        ...
        ftrs = pd.read_csv(emb_file, sep='\s+', skiprows=1, header=None, index_col=0)
        print(f"Embeddings read successfully, shape: {ftrs.shape}")
        ...
        with open(emb_file, 'r') as file:
            content = file.read()
            print(f"Content of the embeddings file:\n{content}")
        return None
